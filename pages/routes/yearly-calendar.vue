<template>
  <div>
    <NuxtLayout name="default">
      <template #sidebar>
        <CachedSearchForm v-model="formData" :allowed-filters="[
          'Source',
          'Cabins',
          'OriginAirport',
          'DestinationAirport',
      ...
      <BButtonSolid v-if="showFilterInfo" data-hs-overlay="#app-sidebar"
      ...
      <div class="flex gap-2 justify-between" v-if="finished">
        ...
        <template v-else>
          <div>
            <h4
              ...
              <Icon name="tabler:plane-departure"
                class="size-6 me-2 text-gray-700" />
              <span class="font-bold">{{
                $t('cached_search_form.labels.outbound') }}</span>
            </h4>
            <FormSelect v-model="inboundFilters.OriginAirport[0]" class="mb-3">
              <template v-for="bucket in inboundAirports" :key="bucket.key">
                <option :label="`${bucket.key}`" :value="bucket.key" />
              </template>
            </FormSelect>
            <YearlyCalendar :start-date="formData?.startDate"
              :end-date="formData?.endDate" :filters="inboundFilters" />
          </div>
          <div>
            <h4
              class="text-lg  text-gray-800 dark:text-white flex align-middle items-center my-2">
              <Icon name="tabler:plane-arrival"
                class="size-6 me-2 text-gray-700" />
              <span class="font-bold">{{ $t('cached_search_form.labels.inbound')
                }}</span>
            </h4>
            <FormSelect v-model="outboundFilters.OriginAirport[0]" class="mb-3">
              <template v-for="bucket in outboundAirports" :key="bucket.key">
                <option :label="`${bucket.key}`" :value="bucket.key" />
              </template>
            </FormSelect>
            <YearlyCalendar :start-date="formData?.startDate"
              :end-date="formData?.endDate" :filters="outboundFilters" />
          </div>
        </template>
      </div>
