<template>
  ...
  <div class="relative lg:size-10 lg:m-2 cursor-pointer z-0 select-none w-fit">
    <svg class="h-full w-full" viewBox="0 0 36 36"
      ...
      <YearlyCalendarDayBar v-if="hasAnyAvailabilityOn('Y')" cabin="Y"
        :is-direct="hasDirectAvailabilityOn('Y')" />
      <YearlyCalendarDayBar v-if="hasAnyAvailabilityOn('W')" cabin="W"
        :is-direct="hasDirectAvailabilityOn('W')" />
      <YearlyCalendarDayBar v-if="hasAnyAvailabilityOn('J')" cabin="J"
        :is-direct="hasDirectAvailabilityOn('J')" />
      <YearlyCalendarDayBar v-if="hasAnyAvailabilityOn('F')" cabin="F"
        :is-direct="hasDirectAvailabilityOn('F')" />
    </svg>
    <div
      class="absolute start-0 end-0 top-0 bottom-0 flex items-center justify-center h-full">
      <span class=" text-center text-xs md:text-md lg:text-lg font-bold
